...
client_socket.connect(("192.168.1.12", 1234))  # Server IP and port
...
class ClientWindow(QMainWindow):
    ...
    def toggle_chat_visibility(self):
        if self.is_chat_hidden:
            code, ok = QInputDialog.getText(self, "Enter Code", "Enter the code to unhide the chat:")
            if ok:
                if code == self.pair_code:
                    ...
                    # Hide emoji, send, and save buttons; show only the "Unhide Chat" and "Dark Mode" buttons
                    for button in [self.input_box, self.emoji_button, self.send_button, self.save_button]:
                        button.hide()
                else:
                    QMessageBox.critical(self, "Invalid Code", "Incorrect code. Chat cannot be revealed.")
        else:
            self.text_browser.hide()
            self.hide_unhide_button.setText("Unhide Chat")
            self.is_chat_hidden = True

            # Show emoji, send, and save buttons; hide the input box and the "Unhide Chat" button
            for button in [self.input_box, self.emoji_button, self.send_button, self.save_button]:
                button.show()
